...
const DICE = (function () {
  ...
  that.parse_notation = function (notation) {
    ...
    while ((res = dr0.exec(no[0]))) {
      ...
      if (res[1] == "") count = 1;
      var type = "d" + res[3];
      if (CONSTS.known_types.indexOf(type) == -1) {
        ret.error = true;
        continue;
      }
  ...
  that.setTheme = function (options) {
    ...
    if (options.label_color !== undefined)
      vars.label_color = options.label_color;
    if (options.label_font !== undefined) vars.label_font = options.label_font;
    if (options.use_avatar_style !== undefined)
      vars.use_avatar_style = options.use_avatar_style;
    if (options.avatar_face_texture !== undefined)
      vars.avatar_face_texture = options.avatar_face_texture;
    if (options.avatar_number_outline !== undefined)
      vars.avatar_number_outline = options.avatar_number_outline;
    if (options.avatar_inner_color !== undefined)
      vars.avatar_inner_color = options.avatar_inner_color;
    if (options.avatar_rim_color !== undefined)
      vars.avatar_rim_color = options.avatar_rim_color;
    if (options.ambient_light_color !== undefined)
      vars.ambient_light_color = options.ambient_light_color;
    if (options.spot_light_color !== undefined)
      vars.spot_light_color = options.spot_light_color;
    if (options.shininess !== undefined)
      vars.material_options.shininess = options.shininess;
    if (options.specular !== undefined)
      vars.material_options.specular = options.specular;
  ...
  function create_dice_materials(face_labels, size, margin) {
    ...
    for (var i = 0; i < face_labels.length; ++i)
      materials.push(
        new THREE.MeshPhongMaterial(
          $t.copyto(vars.material_options, {
